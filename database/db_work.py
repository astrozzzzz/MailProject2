...
DB_NAME = 'database/server_db.sqlite'
...
class DBHandler:
    ...
    def delete_mail(self, sender, addressee, date, title, main_text):
        self.cur.execute(
            f"""DELETE FROM mails WHERE sender = '{sender}' AND addressee = '{addressee}' AND date = '{date}' AND title = '{title}' AND main_text = '{main_text}'""")
        self.db.commit()
